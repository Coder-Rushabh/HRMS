...
class employee:
    def __init__(self,root):
        self.root= root
        self.root.title("Add New Employee")
        ...
        txt_id = Label(self.root, text="ID", 
    ...
    #################################
        ...
        self.txt_id = Entry(self.root, textvariable=self.var_id,
                                 font=("goudy old style",15), 
                                 bg = "#f0f0f0",fg="black")
        self.txt_id.place(x=110,y=90, width=200)

        txt_name = Entry(self.root, textvariable=self.var_name,
                                 font=("goudy old style",15), 
                                 bg = "#f0f0f0",fg="black").place(x=110,y=160,width=200)
        ...
        try:
            if self.var_name.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from employee where name=?",(self.var_name.get(),))
                        con.commit()
    ...
    def save(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Name already exist", parent=self.root)
                else: 
                    cur.execute("insert into employee (id,name,email,gender,dob,mobile,doj,city,state,position,address) values(?,?,?,?,?,?,?,?,?,?,?)",(
                        self.var_id.get(),
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
        ...
        try:
            if self.var_name.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("update employee set name=?,email=?,gender=?,dob=?,mobile=?,doj=?,city=?,state=?,position=?,address=? where id=?",(
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
                        self.var_dob.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Employee detail(s) update sucessfully",parent=self.root)
                    self.show()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to {str(ex)}")
